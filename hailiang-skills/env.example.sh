...
# 仅在 SKILL.md 出现资料/禁止性约束且本轮可能输出结论时使用的轻量预检。
# 超时、异常或无效 JSON 会自动放行原回复，并写入诊断事件。
export HAILIANG_REFERENCE_PREFLIGHT_TIMEOUT_S="8"
export HAILIANG_REFERENCE_PREFLIGHT_MAX_TOKENS="512"
