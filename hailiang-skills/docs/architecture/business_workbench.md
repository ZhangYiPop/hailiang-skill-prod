...
启动自动回灌默认关闭。第一阶段用 `scripts/migrate_business_catalog.py` 预检并幂等迁入数据库；`HAILIANG_BUSINESS_CONFIG_SOURCE=filesystem|database` 用于文件版/数据库版能力对照，两种来源不会混合。数据库模式从当前发布独立构建 Bundle，缺少 `general_chat` 或 `career_plan_entity` 时明确启动失败。验收完成后的第二阶段删除业务目录和文件加载分支。
...
- `HAILIANG_BUSINESS_CONFIG_SOURCE`：第一阶段来源选择，`database` 模式禁止文件补字段。
