...
- 新增 `guides/AGENT_SYSTEM_BRIEFING_SPEECH.md`，用于向业务管理层和技术管理层介绍当前 Agent 系统
- 讲稿按高层口径梳理了系统定位、统一调度架构、专项能力协同、按需调用机制，以及生涯规划能力的体现方式
- 单独补充了“已实现效果/可分享效果”和“待完善功能 Top 3”的汇报表达，便于直接用于口头汇报
- 明确规避不适合对上展开的底层运行时细节，避免在汇报中暴露过深实现名词

#### README 补充管理汇报材料入口

- 在 `README.md` 中新增“管理汇报材料”章节
- 补充 Agent 系统向上汇报讲稿入口，并说明该文档适合用于高层架构说明和能力介绍

### 2026-09-04

#### SSE v2 专家上下文版本字段改为前端可选

- `expected_branch_version` 和 `expected_selection_version` 不再要求前端强制维护和回传
- 服务端完成 session/profile 分支恢复后，对缺省字段使用当前权威版本；响应仍返回完整 `expert_context`
- 显式传入旧版本时继续返回 `409 EXPERT_CONTEXT_STALE`，不削弱已有新客户端的并发保护
- 补充缺省版本的解析和 API 回归测试

#### 修复专家团唤醒后手工调试请求 422 的排查文档

- 确认缺少 `input.expert_context.expected_branch_version` 会在 Pydantic 请求校验阶段直接返回 422，不会进入专家团运行时
- 补充专家上下文严格契约中的完整继续对话示例，包含 `expected_branch_version` 和 `expected_selection_version`
- 更新 `README.md`，明确手工调试和旧版前端需要使用最近一次 SSE `state` 返回的专家上下文版本
- 保留本次调试记录，便于核对请求字段与运行时证据

#### 修复主协调专家后续轮次漏生成转交卡

- 主协调专家每轮新消息都会重新判断成员专家是否更适合承接
- 明确要求命中成员职责时必须调用 `propose_member_handoff` 生成新的结构化转交卡
- 禁止未调用转交工具时仅在正文中输出 `@专家` 或“建议转交”表达
- 增加 `handoff_tool_calls` 和 `structured_handoff` 运行事件字段，便于核对模型工具调用与最终卡片状态
- 同步强化学生成长专家团与升学规划专家配置中的每轮转交规则
- 更新专家团接入 guide，明确主协调专家在后续轮次仍需重新判断并生成新的转交卡

### 2026-09-03

#### 业务调试台支持递归导入专家团配置包到对象库

- 业务调试台新增“递归导入到工作台”入口，可直接导入由发布中心导出的 ZIP 配置包
- 导入专家团包时会按依赖闭包自动回灌 `expert_team -> expert -> skill`，而不是只进入生产暂存区
- 导入冲突策略调整为按 `object_type + object_key` 复用已有对象：内容一致则复用已有版本，内容不同则在原对象下新增导入版本
- 导入后的 Skill `references/`、`scripts/` 等文件会一并落库，后续仍可继续编辑、调试、发布
- 保留现有 `/deployment/v1/imports` 的“生产暂存导入”链路不变，继续用于部署激活与回滚
- 更新 `README.md`，补充业务调试台两种导入链路的语义区别
